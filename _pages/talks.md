  ...
  {% for talk in talks_sorted %}
    <div class="talk-entry"> 
      <p>
        ...
        {%- if talk.venue -%}
          . {% if talk.status == "upcoming" %}To be held at{% else %}Held at{% endif %} <em>{{ talk.venue }}</em>
          {%- if talk.location %} in {{ talk.location }}{% endif -%}
        {%- endif -%}
        {%- if talk.video %}. A video recording is available on <a href="{{ talk.video }}" target="_blank">YouTube</a>{% endif -%}.
      </p>
    </div>
  {% endfor %}
